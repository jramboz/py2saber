...
class Saber_Controller:
    ...
    @staticmethod
    async def create(port: str=None, gui: bool = False, loglevel: int = logging.ERROR) -> 'Saber_Controller':
    ...
    async def send_command(self, cmd: bytes) -> None:
        ...
        if len(cmd) <= self._CHUNK_SIZE: # Send the whole thing at once
            ...
            while pos < len(cmd):
                ...
                self._ser.write_async(cmd[pos:pos+self._CHUNK_SIZE])
    ...
    async def erase_all_files(self, progress_callback: callable = None) -> None:
        ...
        if await self.saber_is_ready():
            ...
            response = await self.read_line() # "Erasing Serial Flash, this may take 20s to 2 minutes\n"
    ...
    async def auto_assign_sound_effects(self):
        ...
        files = (await self.list_files_on_saber()).keys() # Just the keys because we only need the filenames
...
async def main_func():
    ...
    try:
        ...
        sc = await Saber_Controller.create(loglevel=logging.DEBUG if args.debug else logging.ERROR)
